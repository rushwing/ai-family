...
- [gemini-r2][2026-06-14] ① 06 状态图数据流绕过鉴权网关、图文不符 → BUG-009；② kid 压测：RAG 注入（检索网页夹带“忽略安全策略”）+ 输出过滤被 Base64 等编码绕过则红线破——强化 BUG-005（网关消毒须做指令/数据定界 + 输出分类器抗编码），并汇入 REQ-009 对抗测试集。human-001 裁决：待定
