...
date: 2026-06-11
deciders: [human-001]
informed_by: [codex-005, gemini-002, gemini-反驳, gemini-r2]
supersedes: null
linked_reqs: [REQ-003]
---
...
- [claude-code-001][2026-06-14] BUG-009 已修复（决策一致性缺陷，同 BUG-008 类）：06 §2 状态图新增「🔒 工具侧鉴权边界」带——显式画出 `写操作节点 → MCP 网关（验签/白名单/工具侧 JWT）→ PG(RLS)/MinIO`，对齐本 ADR 信任边界与 02 §3，图中不再有绕过网关的数据流。BUG-009 → resolved，本 ADR 决策一致性闭环。（BUG-005/REQ-004/REQ-009 属实现期加固，按 adr-standard §4.1 由下游 REQ 的 done 门禁覆盖，不阻塞本 ADR accepted。）
